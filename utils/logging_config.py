...
def log_download_attempt(logger: logging.Logger, user_id: int, spotify_url: str, quality: int):
    """
...
def log_download_success(logger: logging.Logger, user_id: int, filename: str, file_size: int):
    """
    ...
    log_user_action(
        ...
        user_id, 
        "Download success", 
        f"File: {filename} Size: {size_mb:.2f}MB"
    )


def log_download_error(logger: logging.Logger, user_id: int, error: str, spotify_url: str):
    """
    Log download errors.
    ...
    log_user_action(
        ...
        user_id, 
        "Download error", 
        f"Error: {error} URL: {spotify_url[:50]}..."
    )