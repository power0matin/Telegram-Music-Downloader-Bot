...
from utils.logging_config import setup_logging, log_download_attempt, log_download_success, log_download_error
...
class SpotifyDownloader:
    ...
    def __init__(self, download_dir: str = None):
        ...
        self.max_file_size = config.max_download_size_mb * 1024 * 1024  # Convert to bytes
    ...
    def _prepare_download_command(self, spotify_url: str, output_dir: str, quality: int) -> List[str]:
        """
        ...
        command = [
            ...
            "--output", output_dir,
            "--format", "mp3",
            "--overwrite", "skip",  # Skip if file already exists
        ]
        
        return command
    
    def _execute_download(self, command: List[str], timeout: int = 300) -> subprocess.CompletedProcess:
        """
        ...
        try:
            for filename in os.listdir(download_dir):
                if filename.endswith(".mp3"):
                    ...
                    logger.debug(f"Processed file: {filename} ({file_info['size_mb']:.2f}MB)")
    ...
    def download(self, spotify_url: str, user_id: int, quality: int = None) -> DownloadResult:
        """
        ...
        command = self._prepare_download_command(spotify_url, user_download_dir, quality)
        ...
        try:
            ...
            for file_info in files:
                log_download_success(logger, user_id, file_info['filename'], file_info['size'])
    ...
    try:
        ...
        if not deps['ffmpeg']:
            ...
            try:
                ...
                bot.send_message(user_id, messages.get('ffmpeg_install_failed', error=str(e)))
                return
        ...
        for file_path in result.files:
            try:
                ...
                bot.send_message(
                    user_id, 
                    messages.get('file_send_error', error=str(e))
                )
