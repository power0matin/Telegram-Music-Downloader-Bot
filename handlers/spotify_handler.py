...
from utils.spotify_utils import validate_spotify_url, sanitize_spotify_url, get_spotify_metadata
...
def register_spotify_handler(bot: TeleBot):
    ...
    def handle_spotify_url(message: Message):
        ...
        if not is_allowed:
            log_user_action(logger, user_id, "Rate limited", f"Wait: {time_until_allowed:.1f}s")
            bot.reply_to(
                message,
                messages.get('rate_limit_exceeded')
            )
            return
    ...
    @bot.message_handler(func=lambda message: not _is_spotify_message(message) and not message.text.startswith('/'))
